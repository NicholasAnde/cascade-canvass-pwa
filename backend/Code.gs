/* Cascade Canvass — Sheets + Email (Auto-Formatting, No Drive), with Lead Status */
...
const SHARED_SECRET = 'CHANGE_ME'; // match app.settings.json
...
function doPost(e){
  try{
    ...
    if ((body.type||'').toLowerCase()==='lead'){
      ...
      sh.appendRow([lead.date,lead.time,lead.name,lead.phonePretty,lead.phoneE164,lead.email,lead.address,lead.service,lead.urgency,lead.timeline,lead.budget,lead.status,lead.notes,body.rep||'',body.source||'PWA',lead.emailLink,lead.callLink]);
      fmtLeads(sh); sendLeadEmail(lead, body.photos); return _json({ok:true});
    } else {
      const v=normVisit(body,dateLocal,timeLocal);
      const sh=sheet(ss,SHEETS.VISITS,visitHeaders());
      sh.appendRow([v.date,v.time,v.outcome,v.objection,v.address,v.notes,body.rep||'',body.source||'PWA']); fmtVisits(sh);
...
function normLead(d,dateStr,timeStr){
  ...
  const fmt=ds=>ds.length===10?{pretty:`(${ds.slice(0,3)}) ${ds.slice(3,6)}-${ds.slice(6)}`,e164:`+1${ds}`}:(ds.length>10&&ds.length<=15?{pretty:`+${ds}`,e164:`+${ds}`}:{pretty:'',e164:''});
  ...
  return { date:dateStr,time:timeStr,name:title(d.name),phonePretty:p.pretty,phoneE164:p.e164,email:String(d.email||'').trim(),
           address:title(d.address),service:d.service||'',urgency:d.urgency||'',timeline:d.timeline||'',budget:d.budget||'',status:String(d.status||'New'),
...
function sendLeadEmail(lead,photos){ const to=PropertiesService.getScriptProperties().getProperty('EMAIL_TO')||DEFAULT_EMAIL_TO; if(!to) return;
  const subject="New Lead (Cascade Lead App)"; const esc=s=>String(s).replace(/[<>&]/g,c=>({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]));
  const html=`<div style="font:14px/1.5 -apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#222"><h2>New Lead</h2><div><b>Name:</b> ${esc(lead.name)}</div><div><b>Status:</b> ${esc(lead.status)}</div><div><b>Phone:</b> ${esc(lead.phonePretty)} <span style="color:#888">(${esc(lead.phoneE164)})</span></div><div><b>Email:</b> ${esc(lead.email)}</div><div><b>Address:</b> ${esc(lead.address)}</div><div><b>Service:</b> ${esc(lead.service)}</div><div><b>Urgency:</b> ${esc(lead.urgency)}</div><div><b>Timeline:</b> ${esc(lead.timeline)}</div><div><b>Budget:</b> ${esc(lead.budget)}</div><div><b>Notes:</b><br/>${esc(lead.notes).replace(/\n/g,'<br/>')}</div></div>`;
...
function leadHeaders(){ return ['Lead Date','Lead Time','Name','Phone (Pretty)','Phone (E.164)','Email','Address','Service','Urgency','Timeline','Budget','Status','Notes','Rep','Source','Email ⤳','Call ⤳']; }
...
function fmtLeads(sh){ const h=leadHeaders(); const c={}; h.forEach((x,i)=>c[x]=i+1); const rows=Math.max(1,sh.getLastRow()-1);
  ...
  sh.setColumnWidth(c['Email'],200); sh.setColumnWidth(c['Address'],260); sh.setColumnWidth(c['Status'],110); sh.setColumnWidth(c['Notes'],320);
}
