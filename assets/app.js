...
function confirmVisit(outcome){
  ...
  const ok = confirm(`Log "${outcome}" at:\\n${addr}?`); if(!ok) return; postVisit_geo(outcome);
}
...
async function saveLead(){
  ...
  const b={ type:'lead', date:todayISO(), time:new Date().toISOString(),
    ...
    budget:el('#l_budget').value, notes:(el('#l_notes').value||'').trim(), rep:S.rep||'', source:'PWA', photosBase64:photos };
...
function renderMapToday(){
  ...
  function getPts(){ const all=(S.visitsLog||[]).filter(v=>typeof v.lat==='number'&&typeof v.lon==='number'); return range==='today'?all.filter(v=>(v.date||'').slice(0,10)===t):all.filter(v=>(v.date||'')>=wk); }
  function draw(){ const pts=getPts(); if(!window.L){ showToast('Map lib missing','error'); return; } const map=L.map('map'); const start=pts[0]?[pts[0].lat,pts[0].lon]:[45.64,-122.67]; map.setView(start, pts[0]?16:12); L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map); if(pts.length){ const b=[]; pts.forEach(p=>{ const m=L.marker([p.lat,p.lon]).addTo(map); m.bindPopup(`<b>${p.address||''}</b><br/><small>${p.outcome||'Visit'} • ${new Date(p.time||'').toLocaleTimeString()}</small>`); b.push([p.lat,p.lon]); }); if(b.length>1) map.fitBounds(b,{padding:[20,20]}); } }
  setTimeout(draw,0);
}
